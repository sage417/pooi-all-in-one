...
pub struct StdOutboundDatagram(UdpSocket);
pub struct StdOutboundDatagramRecvHalf(Arc<Mutex<UdpSocket>>);
pub struct StdOutboundDatagramSendHalf(Arc<Mutex<UdpSocket>>);

/// An inbound datagram simply wraps a UDP socket.
pub struct SimpleInboundDatagram(UdpSocket);
pub struct SimpleInboundDatagramRecvHalf(Arc<Mutex<UdpSocket>>);
pub struct SimpleInboundDatagramSendHalf(Arc<Mutex<UdpSocket>>);
...
impl OutboundDatagram for StdOutboundDatagram {
    fn split(
        ...
        let arc_socket = Arc::new(Mutex::new(self.0));
        (
            Box::new(StdOutboundDatagramRecvHalf(Arc::clone(&arc_socket))),
            Box::new(StdOutboundDatagramSendHalf(Arc::clone(&arc_socket))),
        )
    }
}
...
impl OutboundDatagramRecvHalf for StdOutboundDatagramRecvHalf {
    async fn recv_from(&mut self, buf: &mut [u8]) -> io::Result<(usize, SocksAddr)> {
        match self.0.lock().await.recv_from(buf).await {
            Ok((n, a)) => Ok((n, SocksAddr::SocketAddr(unmapped_ipv4(a)))),
            Err(e) => Err(e),
        }
    }
}
...
impl OutboundDatagramSendHalf for StdOutboundDatagramSendHalf {
    async fn send_to(&mut self, buf: &[u8], target: &SocksAddr) -> io::Result<usize> {
        match target {
            SocksAddr::SocketAddr(a) => self.0.lock().await.send_to(buf, a).await,
            SocksAddr::DomainNameAddr(domain, port) => Err(io::Error::new(
                io::ErrorKind::Other,
    ...
    async fn close(&mut self) -> io::Result<()> {
        // No-op: The underlying UdpSocket is wrapped in an Arc,
        // and will be automatically dropped when no longer in use.
        Ok(())
    }
}
...
impl InboundDatagram for SimpleInboundDatagram {
    fn split(
        ...
        let arc_socket: Arc<Mutex<UdpSocket>> = Arc::new(Mutex::new(self.0));
        (
            Box::new(SimpleInboundDatagramRecvHalf(Arc::clone(&arc_socket))),
            Box::new(SimpleInboundDatagramSendHalf(Arc::clone(&arc_socket))),
        )
    }
